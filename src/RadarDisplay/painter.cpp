...
void CPainter::updatePPIRadius(int radius)
{
	radiusPPI = radius;
}
...
void CPainter::drawCircles(int x,int y,int step)
{
	...
	//int distantCircleCount=600/step; radiusPPI
	int distantCircleCount=radiusPPI/step;
	//printf("step = %d\n",step);
	int ** pnx = nx;
	int ** pny = ny;
	int scale = 1024;//11.378=360.0*4096.0;
...
{
    int px,py;
    int R = (double)rangePPI/(double)(distantCircleCount);//radiusPPI(double)radiusPPI/(double)(distantCircleCount+1);
